...
DESCRIPTION
    ...
    "trim" filter than the default. Rather than trimming *all*
    leading/trailing whitespace, we trim all leading *blank lines* and all
    trailing whitespace. In a wikified field, leading spaces on the first
    line could be important so they need to be preserved (while leading
    blank lines aren't important and could be trimmed out).

METHODS
    wiki_trim()
        Returns a filter which trims leading/trailing whitespace in a manner
        more suitable for wikified entry fields; leading blank -lines- are
